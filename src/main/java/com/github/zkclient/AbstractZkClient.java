 ...
 * nodes in ZooKeeper
 */
public abstract class AbstractZkClient<T> implements Watcher, IZkClient<T> {

    private final static Logger LOG = Logger.getLogger(AbstractZkClient.class);
    ...
    public AbstractZkClient(String zkServers, int sessionTimeout, int connectionTimeout, ZkSerializer<T> zkSerializer) {
        this(new ZkConnection(zkServers, sessionTimeout), connectionTimeout, zkSerializer);
    }

    public AbstractZkClient(IZkConnection zkConnection, int connectionTimeout, ZkSerializer<T> zkSerializer) {
        _connection = zkConnection;
        _zkSerializer = zkSerializer;
        connect(connectionTimeout, this);
    }

    public void setZkSerializer(ZkSerializer<T> zkSerializer) {
        _zkSerializer = zkSerializer;
    }
     ...
     * @throws RuntimeException if any other exception occurs
     */
    public void createEphemeral(final String path, final T data) throws ZkInterruptedException,
            IllegalArgumentException, ZkException, RuntimeException {
     ...
     * @throws RuntimeException if any other exception occurs
     */
    public String createEphemeralSequential(final String path, final T data) throws ZkInterruptedException,
            IllegalArgumentException, ZkException, RuntimeException {
    ...
    public boolean delete(final String path) {
        try {
            retryUntilConnected(new Callable<T>() {
    ...
    private T derializable(byte[] data) {
        if (data == null) {
            return null;
        }
    ...
    protected T readData(final String path, final Stat stat, final boolean watch) {
    ...
    public Stat writeData(String path, T object) {
    ...
    public Stat writeData(final String path, T datat, final int expectedVersion) {
